...
**同じ現象はビット列領域でも公表済み** —— [Lobo & Bazargani, *When Hillclimbers Beat Genetic Algorithms in Multimodal Optimization*, Evolutionary Computation 30(4):535-559, 2022](https://direct.mit.edu/evco/article-abstract/30/4/535/111270/When-Hillclimbers-Beat-Genetic-Algorithms-in)。
**【2026-09-13 その119 で訂正】OA 版は在る: [arXiv:1504.06859](https://arxiv.org/abs/1504.06859)（v2, 2022-06-09）。**
その118 は「arXiv にも題名一致の preprint が見つからない」と記録したが**誤り**で、題名は完全一致する。**本文はまだ未取得**（この環境は `arxiv.org` も egress 403）。
**抄録で確定している範囲**（その119 が検索経由で確認）: 対象は**ビット列領域の multimodal problem generator**、
理論解析は **"uniformly distributed equal-height instances"** に対する multistart next ascent hillclimbing の平均時間、
結論は「niching と交配制限を入れた EA は hillclimbing 戦略に対抗できない」、機序は**予想**（"conjecture"）として
**"the lack of structure in the space of local optima on instances of this problem class"**。
**＝ 予想の前提は「一様配置」で、その119 はこの suite の群 A が一様でない（塊）ことを 8/8 で測っている。
前提が成り立たない側で何が起きるかは、この論文は述べていない。****詳細と「残っている主張」は [research_loop.md の「占有されている領域」](research_loop.md#この研究のゴール毎回着手する問いをここに照らすこと)（2026-09-12 の 10 件目）と [status.md](status.md) の判断 (1)。**
...
#### 公表 PR の実額（その71 で取得。ここが案 (A) の headroom そのもの）

サンドボックスの egress は `arxiv.org` / `pmc` / `semanticscholar` / `dl.acm.org` を CONNECT 403 で塞ぐが、
**GitHub Actions の runner は塞いでいない**（`run.yml` の `mode: fetch_refs`、5 本の PDF を 18 秒で取得）。
以下は取得した本文からの転記で、**この環境での再実行値ではない**。

**主表: PR@ε=1e-5・正規予算・50 run**（[HillVallEA GECCO'18, arXiv 1810.07085](https://arxiv.org/pdf/1810.07085) 表 4。
同論文 5 節が「suite の 5 水準平均ではなく最高精度 1e-5 のみを使う」と明記しており、
**本テーマの判定水準（ε ≤ 1e-3）に最も近い唯一の公表表**）:

| PR@1e-5 | F11 | F12 | F13 | F14 | F15 | F16 | F17 | F18 | F19 | F20 |
|---|---|---|---|---|---|---|---|---|---|---|
| RS-CMSA | 1.000 | 1.000 | 1.000 | 0.780 | 0.748 | 0.667 | 0.688 | 0.667 | 0.505 | **0.465** |
| HillVallEA-AMu | 1.000 | 1.000 | 0.973 | 0.783 | **0.750** | 0.673 | **0.745** | 0.657 | **0.512** | 0.318 |
| 6 手法の最良 | 1.000 | 1.000 | 1.000 | 0.793 | 0.750 | 0.677 | 0.745 | 0.667 | 0.512 | 0.465 |
| **headroom** | **0** | **0** | **0** | 0.207 | 0.250 | 0.323 | 0.333 | 0.333 | 0.488 | 0.535 |

**F11-F13 の headroom はゼロ**（既に PR 1.000）。**案 (A) の実体は F14-F20 の 7 関数**であって F11-F20 ではない。

**副表: PR を 5 水準（1e-1〜1e-5）で平均・正規予算・50 run**（[HillVallEA19 報告, arXiv 1907.10988](https://arxiv.org/pdf/1907.10988) 表 2、
6 手法 = NEA2+ / RLSIS / RS-CMSA / SDE-Ga / HillVallEA18 / HillVallEA19）。**主表と混ぜないこと**（水準の定義が違う）:
最良は F11 1.000 / F12 1.000 / F13 1.000 / F14 0.923 / F15 0.750 / F16 0.723 / F17 0.750 / F18 0.667 / F19 0.593 / F20 0.482。

**残差の正体は「K のうち届かない 2 個」で、名指しされている。** 上記 2 本と HillVallEA19 報告が同じことを書く:
**CF4 系（F15/F17/F19/F20）の Weierstrass 2 峰はどの手法もどの次元でも取れない**、
**CF3 系（F18）は全手法が 0.667 = 4/6 で止まる**、F20 は Griewank 峰も取れない。
0.667 = 4/6、0.750 = 6/8 という値が複数手法で一致するのはこのため ＝ **PR の差は「被覆の広さ」ではなく
「特定の 2 個の難しい峰に到達できるか」で決まっている**。K が 6-8 しかないことと合わせると、
**本テーマが測ってきた被覆（着地先分布の偏り）とは律速が違う**。

**予算 10^9（正規の 1000 倍超）でも全問全解には届かない**と 1810.07085 の 5.5 節が報告している
＝ **F14-F20 の headroom は「予算不足」ではなく未解決**。案 (A) の価値はここにあり、同時にリスクもここにある。

**参考にしてはいけない 2 本**（取得はしたが headroom の根拠に使えない）:
- [RLEMMO, arXiv 2404.08242](https://arxiv.org/pdf/2404.08242) 表 2 は **PR@1e-4 だが maxFEs を全 20 問一律 50000** に切っている
  （正規予算の 1/4〜1/8）。F8 0.124 / F9 0.211 と、正規予算の公表値（0.87-0.99）から桁で外れる。
  **その50 の「1/10 予算の順位は正規予算に外挿できない」がそのまま当たる。**
  ただし**低予算帯の対照としては使える**（50000 評価での F9 = VNCDE 0.177 / EMO-MMO 0.309 / NBNC-PSO-ES 0.083 /
  PNF-PSO 0.073 / RLEMMO 0.211。本プロジェクトの 1/10 予算 40000 評価の N09 と同じ帯）。
- [arXiv 2511.17571](https://arxiv.org/pdf/2511.17571) 表 6/7 は **予算 1000 評価・pop 30**（正規の 1/400）で、
  さらに**関数番号が公式と違う**（同論文表 4 は f7 = Shubert 3D / f8 = Vincent 2D と公式の F7/F8 を入れ替え、
  F14-F20 の D の並びも公式と一致しない）。**F 番号で他論文と突き合わせてはいけない実例。**

#### NMMSO 自身の公表 PR 表（その134 で取得。キュー 2 の照合先）
...
**PR@1e-5（表 IV の最終行。NMMSO 列のみ転記）**:

| PR@1e-5 | F1-F5 | F6 | F7 | F8 | F9 | F10 | F11 | F12 | F13 | F14 | F15 | F16 | F17 | F18 | F19 | F20 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| NMMSO | 1.000 | **0.000** | 1.000 | 0.980 | 0.913 | 1.000 | 1.000 | 0.998 | 0.990 | 0.703 | 0.668 | 0.660 | 0.538 | 0.633 | 0.443 | 0.178 |

同表 V の全問題・全水準平均は **NMMSO 0.8271（中央値 0.9933）** で、CEC2013 競技 15 手法中の最高。

**本リポジトリの関数名との対応**（`core/benchmarks.py:627-687` の登録と公式番号は一致している）:
**N04=F4 / N05=F5 / N06=F6 / N07=F7 / N08=F8 / N09=F9 / N10=F10**、
**N18-CF3-10D=F18 / N19-CF4-10D=F19 / N20-CF4-20D=F20**。
＝ **キュー 2 の照合先は N18 0.633 / N19 0.443 / N20 0.178。**
...
**丸写しではない**（F6 0.660 対 0.997、F8 0.897 対 0.981、F9 0.978 対 0.917、F17 0.466 対 0.538 とずれる関数がある）。
**＝ キュー 2 が照合する 3 関数については、独立でない可能性を残したまま 2 本の出典が ±0.02 で揃っている。**

**F6 の 0.000 は手法の性質ではなく suite 側の性質**として読むこと ——
同論文は「**No algorithm found any solutions at the ϵ = 10−5 level on F6**」と書いており、
表 IV では **NMMSO / N-VMO / dADE の 3 手法とも F6 の 1e-4 で 0.997-0.984、1e-5 で揃って 0.000**。
1 水準で 3 手法同時に落ちる崖は採点側の分解能（Shubert 2D は f\* = 186.73）を疑うべき形で、
**この 1 点を「配線が違う」の証拠に使ってはいけない。**
