...
**【2026-09-12 その117 で決着】** この腕（採用形は `hunt_level_tol = 1e-5 / max(f_init_scale, 1e-12)`
＝ c = 1.0）は **BBOB-24 dim2 × 20 seed × 2 予算 = 960 セルで `best_f` と 1e-10 到達評価回数がともに一致**
（その47）＝ **上の「gate は構造的に盲目」は 5000 評価単独の話で、2 予算 ＋ `evals_succ` まで見た形では通っている。**
**判定は「採用可」**（下記の 5 件の表）。**ただし `mceso.py` の既定はこの回も変えていない** ——
案 (C) の「既定値も不変」と、実行ルーチンに既定変更の権限が無いため。

## 滞留していた採用候補 6 件の採否（2026-09-12 その117, **判断は確定 / 既定は未変更**）

方針欄 2026-09-11 のユーザー決定 (3) の手順（ゲートは **`best_f` バイト一致**、`CLAUDE.md` の pin
...
|---|---|---|---|
| `c=1.0` | `hunt_level_tol` の eps 相対化（`mceso_rel_level.py`） | **採用可** | 960/960 `best_f` 一致 ＋ `evals_succ` 480/480 一致（その47） |
| `_sig10` | `exhausted_sigma_tol` 1.5 → 1.0 | **採用可** | 960/960 一致、分岐した 358 run すべてで分岐が答えの後（その56） |
| `_fl08` | `sigma_floor_ratio` 1e-6 → 1e-8 | **採用可** | バイト一致しない（81/960）が **pin は改善**（SR 0.9208 → 0.9375、`evals_succ_mean` 1230.4 → 1216.0、失った SR セル 0）（その56） |
| `soltrim_rho` | `sol_trim_mode="rho"`（`mceso_sol_archive.py`） | **採用可** | **評価履歴がバイト一致**（`differ`=0、hunt ダンプまで一致。その60） |
| `commit_place_r010` | `commit_sigma_mode="place"` / ratio 0.1（`mceso_commit_reseed.py`） | **採用可（ゲートの上で）** | **その117 で初の BBOB gate**: 480/480 セルで `best_f` / `n_evals` / `success` 一致、pin 同値。**代償は残る**（深さ律速の関数では有償、その41・その42） |
| 合成腕（上記 4 件同時 ＝ `comp4`） | `diagnose_niching.py` の `comp4` / `comp4_off` | **N06 / N08 では採用可の側。N09 は未測定** | N06 PR@1e-5 **0.7130 → 0.9861**（12/0/0、p=4.9e-4）。恒等検査 `comp4_off` = base（7 列 × 8 セル） |

**既定を変えたときの代償**（ユーザー決定 (3) の必須条件への回答）: **MC-ESO は案 (C) 以降ベースライン側**なので、
**どれか 1 つでも既定にした時点で `docs/` と `analysis/` の MC-ESO の数値はすべて旧既定のものになる**
（その106 / その107 / その112 / その113 / その115 / その116 の新 suite 値、CEC2013 の N04-N10 と F14-F20、
`CLAUDE.md` の pin、この環境の base 0.9208 / 677.7）。**取り直すまで新旧を混ぜて引用できない。**
