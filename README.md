...
1. ニッチ選択でエリートを特定
   ...
   └─ ※上位5個体が探索空間の5%以内に集中 **かつ** best_f < 1e-3（大域最適解付近）の場合のみ
      niche_radius_min を使用 → 局所最適解への収束時には niche を維持（多峰性関数の脱出を妨げない）
...
4. 子個体の生成（死亡数だけ生成）
   ...
   │       scale       = sigma_min_ratio ^ combined   ← 指数減衰
   │       ※ log_quality=0（悪い個体）は combined=0 → scale=1（常に全力探索）
   │       ※ age は log_quality が高いときのみ combined を増幅（良い老個体だけ fine-tune）
...
#### log スケール品質に基づく探索半径（近傍感染）

```
log_quality = clip((log10(f_max_pop) − log10(f_親)) / log10_spread, 0, 1)
age_ratio   = 親のage / lifespan        # 0=若い, 1=老い
combined    = log_quality × (0.7 + 0.3 × age_ratio)
σ_i         = σ × sigma_min_ratio ^ combined
```

| 状態 | log_quality | combined | σ_i |
|------|-------------|----------|-----|
| 悪い個体（任意年齢） | ≈ 0 | ≈ 0 | σ（常に全力探索、局所最適解から脱出可能） |
| 良い + 若い | ≈ 1 | ≈ 0.7 | σ × 0.16（中程度の fine-tune） |
| 良い + 老い | ≈ 1 | ≈ 1.0 | σ × sigma_min_ratio（精密探索） |

**線形品質との差**: 線形では f=0.001 と f=0.01 が集団内相対ランクに依存していたが、
log スケールでは 10 倍の差を正確に区別する。局所最適解にいる「集団内 best」個体でも
f が大きければ `log_quality` が下がり → sigma が保たれ → 脱出できる。

**なぜ `combined = log_quality × (0.7 + 0.3 × age)` か**:
年齢は log_quality が高い（本当に良い位置）ときだけ combined を増幅する。
悪い個体がいくら老いても combined=0 のまま → sigma 最大 → 死ぬ前に遠くへ探索。
