...
| 探索範囲の制御 | sigma を世代単位でリセット or 乗算 | 親の **fitness + age** に応じて局所 sigma をスケーリング。空気感染は集団の空間的多様性に応じた適応半径を使用 |
...
4. 子個体の生成（死亡数だけ生成）
   ...
   │       scale    = sigma_min_ratio + (1 - combined) × (1 - sigma_min_ratio)
...
combined  = 0.5 × quality + 0.5 × age_ratio
σ_i       = σ × (sigma_min_ratio + (1 - combined) × (1 - sigma_min_ratio))
```
...
- 集団が収束（diversity → 0）: `σ_air` → `σ × air_sigma_max`（大きく飛ぶ）
- 集団が分散（diversity → 1）: `σ_air` → `σ × air_sigma_min`（近くに留まる）
...
Hansen et al. (2009) による標準スイート。すべて大域最適解は1つで、`f(x) - f_opt` が返るため最小値は 0。

| グループ | 関数 | 特徴 |
|---|---|---|
| separable | F01–F05 | 変数間の相互作用なし |
| moderate-cond | F06–F09 | 中程度の条件数 |
| ill-cond | F10–F14 | 高条件数・単峰性 |
| multimodal | F15–F19 | 多くの局所最適解・適切な大域構造 |
| weak-structure | F20–F24 | 多くの局所最適解・弱い大域構造 |

### カスタム関数（複数大域最適解）

BBOB は大域最適解が1つの関数のみを含む。VSO のニッチ選択・マルチコロニー機能を評価するため、複数の等価な大域最適解を持つ関数を独自に追加している。

| 関数 | 大域最適解数 | 最適値 | 探索範囲 |
|---|---|---|---|
| **C01-Himmelblau** | **4** | 0 | [-5, 5] |
| **C02-SixHumpCamel** | **2** | 0（シフト済み） | [-2, 2] |

**C01-Himmelblau**: `f(x,y) = (x²+y-11)² + (x+y²-7)²`  
4つの等価な大域最適解（(3,2), (-2.805,3.131), (-3.779,-3.283), (3.584,-1.848)）を持つ。

**C02-SixHumpCamel**: 6つの局所最適解のうち2つが大域最適解。
...
評価点が順に積み上がる様子を step=100 eval 単位で描画。各手法の探索密度の変化を確認できる。
...
```
optimization/
├── benchmarks.py   # BBOB 24関数 + カスタム関数の定義（ioh 経由、2D/3D/4D）
...
├── main.py         # エントリーポイント（GitHub Actions 経由で実行）
├── quick_check.py  # ローカル軽量確認スクリプト
├── run.sh          # 実験管理 CLI（trigger / download / quick / list / status）
└── results/
    ├── dim2/
    │   ├── {関数名}.svg            # 関数地形（2D等高線 + 収束曲線 + 3D表面）
    │   ├── {関数名}_runs.gif       # 試行別探索軌跡アニメーション
    │   ├── {関数名}_evals.gif      # 評価点蓄積アニメーション
    │   ├── {関数名}_population.gif # 集団配置推移アニメーション
    │   └── {関数名}_stats.json     # 統計量（mean / std / median / success rate）
