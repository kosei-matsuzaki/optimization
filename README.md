...
| 手法 | 分類 | 本実験での位置づけ |
|---|---|---|
| **VSO** (Virus Spread Optimizer) | 群知能・独自提案 | **提案手法（基本形）** |
| **VSO-A** | VSO 拡張（dormant aging） | **提案手法（変形A）** |
| **VSO-C** | VSO 拡張（replace-not-restore） | **提案手法（変形C）** |
...
6. 仮想呼吸（Virtual Breathing）— n_pop_min > 0 のとき有効
   ...
   └─ 拡大（停滞が深い場合: no_improve ≥ pop_grow_trigger）
       └─ f値が良い dormant を pop_change_by×2 個復活（dormant_mode により復活方法が異なる）
          非エリート active の上位 pop_change_by 個を dormant 化（局所解付近から解放）
       個体数は n_pop_min〜n_pop の範囲で動的に変化
```

**dormant_mode による挙動の違い**

| モード | dormant中の老化 | 再活性化時の処理 | 変形名 |
|---|---|---|---|
| `"freeze"` (デフォルト) | なし（完全凍結） | 旧位置をそのまま復元 | VSO |
| `"aging"` | 毎 iter +1 老化 | 寿命超過個体はランダム再初期化 / 若い個体は旧位置復元 | VSO-A |
| `"replace"` | なし | 旧位置を捨て、現 active 親から offspring を生成 | VSO-C |

- **VSO-A の動機**: freeze のままでは悪い dormant 個体が無限に保存され、再活性化しても探索貢献が低い。老化を与えることで寿命超過個体を自然入れ替えする。
- **VSO-C の動機**: dormant 時点の解を再利用するのではなく、停滞解消時点の現集団の知識から新個体を生成することで、再活性化を実質的な追加探索として機能させる。
...
| `pop_change_by` | 5 | 1回の縮小/拡大で変化させる個体数 |
| `pop_change_cooldown` | 30 | 縮小/拡大後のクールダウン世代数 |
| `dormant_mode` | `"freeze"` | dormant 個体の扱い方（`"freeze"` / `"aging"` / `"replace"`）|
