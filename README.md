...
| **Evals (succ med)** | 成功した run のみを対象とした目標到達評価回数の**中央値**。失敗 run は集計から除外（ERT のような max_evals ペナルティ補正は行わない）。全 run 失敗時は `---` |

`Evals (succ med)` は SR@1e-4 と必ず併せて読む。成功率が低い手法では「たまたま成功した 1 run」が中央値を引き下げて見た目の評価数が良く見える失敗モードがあるため、SR@1e-4 で実際にどれだけの run が成功したかを確認すること。SR@1e-2 と組み合わせれば緩い収束段階の差異も捉えられる。`summary.csv` には参考用に旧来の `ert`（失敗 run を max_evals でペナルティ計上した BBOB 標準値）も併記される。
