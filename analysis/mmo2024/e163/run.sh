#!/usr/bin/env bash
# その163 — キュー 1: **D=20 で `descent_budget` を 2 倍（12500 -> 25000）にした 1 水準を 16 問回す**。
#
#   bash analysis/mmo2024/e163/run.sh [deadline HH:MM UTC]
#
# 回すのは **M01-M16 / D=20 / PIN01 / seed 0 / 正規予算 floor(50000x20) = 100 万 = 16 run**。
# 対照（その151 の `Restart-Lander` 既定 12500、同じ 16 問・同じ次元・同じ instance・同じ seed）は
# **追加評価ゼロ**（`analysis/mmo2024/e151/descents.csv.gz` の保存物を読むだけ）。
#
# 条件は その151 と **`--methods` 以外すべて同一**:
...
run_one() {
  ...
  python3 scripts/niching_baseline.py --funcs "$name" --methods Restart-Lander-25000 \
    --evals-frac 1.0 --seeds 1 --seed-offset 0 --report-rule current \
    --csv "$OUT/by_problem/RL25000_${name}.csv" > "$OUT/by_problem/RL25000_${name}.log" 2>&1
