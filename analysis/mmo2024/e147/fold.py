#!/usr/bin/env python3
"""その147 の片付け — 32 本の降下ダンプを 1 本の `.csv.gz` に、
32 本の driver CSV を 1 本の `driver_summary.csv` に畳む（その128〜その146 と同じ手順）。
...
# `arm` 列は sigma_ratio の実値で持つ（analyze.py が float で読む）。
SIG = {"s005": 0.05, "s020": 0.2}


def fold_descents():
    out = os.path.join(HERE, "descents.csv.gz")
    rows, head = [], None
    for arm in ("s005", "s020"):
        ...
        for fn in sorted(os.listdir(d)):
            ...
            with op(os.path.join(d, fn), "rt") as fh:
                ...
                for r in rd:
                    rows.append([SIG[arm], fn.split("_")[0]] + [r[k] for k in head[2:]])
