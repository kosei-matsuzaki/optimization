#!/usr/bin/env python3
"""e171: 14 本の per-function null ダンプを設定ごとに 1 本へ畳む（`function` 列を足す）。

CLAUDE.md の規約（行単位の生 CSV は `.csv.gz`、per-problem を残さない）と
その128〜その131・その170 と同じ手順。**畳む前後で `analyze.py` の出力が 1 文字も
変わらないことを確認してから元 14 本を消すこと。`rm` はこの script に含めない**
（その131 の事故 —— 畳む script と `rm` を 1 つのコマンド行に並べない）。

使い方: python3 analysis/mmo2024/e171/fold.py
"""
