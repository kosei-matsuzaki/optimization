#!/usr/bin/env python3
"""その153 の片付け — D=5 の 32 本のダンプを 2 本の `.csv.gz` に畳む
（その128〜その152 と同じ手順、`e152/fold.py` の写し。この回は RL の `descents/` と RR の `dumps/` の 2 本）。

**畳むだけ。削除はしない**（`rm` は別コマンドで打つ ＝ その131 の事故の再発防止）。
畳んだ後に `analyze.py` の出力が 1 文字も変わらないことを確認してから消すこと。

使い方: python3 analysis/mmo2024/e153/fold.py
"""
...
if __name__ == "__main__":
    fold_dir("descents", "descents.csv.gz", "_seed0")
