#!/usr/bin/env python3
"""その157（キュー 1）— RR-CMA-ES を D=5 の instance PIN02/03/04 で回す。

**その155 の `run_rrcma_cov.py` をそのまま写した。変えたのは出力先（`__file__` 由来で e157）だけ。**
`cov` は既定の 20 で呼ぶ ＝ **その153（PIN01）と 1 ビットも違わない配線**。
