...
class BasicFun:
    ...
    def schwefelN26(x,hGO):
        ...
        P=p1+p2;
        g=1.5*P+np.sum(-y*np.sin(np.abs(y)**.5))+fshift*D;
    ...
    def weierstrass(x,hGO):    
        ...
        b=3.0; # a higher value makes it makes it more rugged-default is 3
        k=np.arange(21,dtype=float); # level of optima
        term1= (2*np.pi*b**k).reshape(-1,1) @ np.atleast_2d(x+0.5); # size=21xD
        h1=np.atleast_2d(a**k) @ np.cos(term1);
        h2=np.sum(a**k*np.cos(np.pi*b**k));
        P= np.sum(  (np.abs(x)-.5)**1 *(np.abs(x)>.5) );
        f0=np.sum(h1)-D*h2+P;
